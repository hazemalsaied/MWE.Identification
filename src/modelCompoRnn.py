...
class Network:
    ...
    def __init__(self, corpus):
        ...
        buffTokEmb = Embedding(len(tokenVocab), configuration['compoRnn']['tokenEmb'], name='B_toksEmb')(bufTokens)
        buffPosEmb = Embedding(len(posVocab), configuration['compoRnn']['posEmb'], name='B_PosEmb')(buffPoss)
        buffTokEmb = Flatten(name='B_toksFlat')(buffTokEmb)
        buffPosEmb = Flatten(name='B_PosFlat')(buffPosEmb)

        s0Tokens = Input((configuration['compoRnn']['stackPadding'],), name='S0_toks')
        s0Poss = Input((configuration['compoRnn']['stackPadding'],), name='S0_Pos')
        inputLayers.append(s0Tokens)
        inputLayers.append(s0Poss)
        s0TokEmb = Embedding(len(tokenVocab), configuration['compoRnn']['tokenEmb'], name='S0_toksEmb')(s0Tokens)
        s0PosEmb = Embedding(len(posVocab), configuration['compoRnn']['posEmb'], name='S0_tPosEmb')(s0Poss)
        s0Emb = keras.layers.concatenate([s0TokEmb, s0PosEmb])
        s1Tokens = Input((configuration['compoRnn']['stackPadding'],), name='S1_toks')
        s1Poss = Input((configuration['compoRnn']['stackPadding'],), name='S1_Pos')
        inputLayers.append(s1Tokens)
        inputLayers.append(s1Poss)
        s1TokEmb = Embedding(len(tokenVocab), configuration['compoRnn']['tokenEmb'], name='S1_toksEmb')(s1Tokens)
        s1PosEmb = Embedding(len(posVocab), configuration['compoRnn']['posEmb'], name='S1_PosEmb')(s1Poss)
        s1Emb = keras.layers.concatenate([s1TokEmb, s1PosEmb])
        if configuration['compoRnn']['gru']:
            s0Rnn = GRU(configuration['compoRnn']['wordRnnUnitNum'], return_sequences=configuration['compoRnn']['rnnSequence'],
                        name='s0Rnn')(s0Emb)
            s1Rnn = GRU(configuration['compoRnn']['wordRnnUnitNum'], return_sequences=configuration['compoRnn']['rnnSequence'],
                        name='s1Rnn')(s1Emb)
        else:
            s0Rnn = LSTM(configuration['compoRnn']['wordRnnUnitNum'], return_sequences=configuration['compoRnn']['rnnSequence'],
                         name='s0Rnn')(s0Emb)
            s1Rnn = LSTM(configuration['compoRnn']['wordRnnUnitNum'], return_sequences=configuration['compoRnn']['rnnSequence'],
                         name='s1Rnn')(s1Emb)
        if configuration['compoRnn']['rnnSequence']:
            s0Rnn = Flatten()(s0Rnn)
            s1Rnn = Flatten()(s1Rnn)
        s0Rnn = Dropout(configuration['compoRnn']['rnnDropout'])(s0Rnn)
        s1Rnn = Dropout(configuration['compoRnn']['rnnDropout'])(s1Rnn)
        lastLayer = keras.layers.concatenate([buffTokEmb, buffPosEmb, s0Rnn, s1Rnn])
        lastLayer = Dense(configuration['compoRnn']['denseUnitNumber'], activation='relu')(lastLayer)
        lastLayer = Dropout(configuration['compoRnn']['denseDropout'])(lastLayer)
...
def train(cls, corpus):
    ...
    optimizer = keras.optimizers.Adagrad(lr=configuration['compoRnn']['lr'], epsilon=None, decay=0.0)
    cls.model.compile(loss='categorical_crossentropy', optimizer=optimizer, metrics=['accuracy'])
    callbacks = [EarlyStopping(patience=configuration['nn']['patience'])] if configuration['nn'][
        'earlyStop'] else []
    history = cls.model.fit(data, lbls,
                            validation_split=.1,
                            epochs=configuration['nn']['epochs'],
                            batch_size=configuration['compoRnn']['batchSize'],
                            callbacks=callbacks,
...
def getIdxs(config):
    ...
    buffElements = 1 + configuration['embedding']['useB1'] + configuration['embedding']['useB-1']
    stackElements = configuration['compoRnn']['stackPadding']
    if config.buffer:
