...
class Transition(object):
    ...
    def getLegalTransDic(self, categorisation=False):
        ...
        if configuration['others']['minimal']:
            if len(config.stack) >= 1 and len(getTokens(config.stack[-1])) >=1:
...
class Configuration:
    ...
    def __str__(self):
        ...
        # + ' Bx=' + self.reduced.getLemma() if self.reduced else ''
